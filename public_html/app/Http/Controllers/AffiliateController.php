...
class AffiliateController extends Controller
{
    ...
    {
        $refferal_user = User::find($id);
        $affiliates = AffiliateUser::where('status', 1)->get();
        if ($request->isMethod('post')) {
            $refferal_user->referred_by = $request->reffered_by;
            $refferal_user->save();
            flash(translate('Refferal user updated successfully'))->success();
            return redirect()->route('refferals.users');
        }
        return view('affiliate.refferal_detail', compact('refferal_user', 'affiliates'));
    }
    ...
    public function users(Request $request)
    {
        ...
        if ($request->keyword != null || $request->status != null) {
            // $affiliate_users = AffiliateUser::paginate(12);
            // join with users table and search by name or email
            if ($request->status != null && $request->keyword) {
                $affiliate_users = AffiliateUser::join('users', 'affiliate_users.user_id', '=', 'users.id')
                    ->where('affiliate_users.status', $request->status)
                ...
                if ($order->user != null) {
                    if ($order->user->referred_by != null) {
                        ...
                        if ($user != null) {
                            ...
                            // add them cap dau tien
                            $userRoot = User::find($user->referred_by);
                            \Log::info("root", [$userRoot]);
                            if ($userRoot) {
                                $affiliate_user_root = $userRoot->affiliate_user;
                                $amount = $amount * 10 / 100;
                                $affiliate_user_root->balance += $amount;
                                $affiliate_user_root->save();
                                // save log
                                $affiliate_log                      = new AffiliateLog;
                                $affiliate_log->user_id             = $user->id;
                                $affiliate_log->referred_by_user    = $userRoot->id;
                                $affiliate_log->amount              = $amount;
                                $affiliate_log->order_id            = $order->id;
                                $affiliate_log->affiliate_type      = 'Tri ân';
                                $affiliate_log->save();
                            }
                        }
                    }
