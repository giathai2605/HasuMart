...
<div class="card">
    <div class="card-header d-flex justify-content-between">
        ...
        <form action="{{route('affiliate.users')}}" method="POST" enctype="multipart/form-data" class="row">
            @csrf
                <select name="status" class="form-control col-md-5">
                    <option value="" selected>{{translate('Filter by status')}}</option>
                    <option value="0">Chưa chấp thuận</option>
                    <option value="1">Đã chấp thuận</option>
                </select>      
                <input type="text" class="form-control col-md-5 mx-auto" name="keyword" placeholder="{{translate('Search')}}">
            <button type="submit" class="btn btn-primary">{{translate('Sort')}}</button>
        </form>
    </div>
    <div class="card-body">
