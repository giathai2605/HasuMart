...
@section('panel_content')
    <div class="card shadow-none rounded-0 border">
        ...
        <div class="card-body">
                            ...
                            <tr>
                                ...
                                <td class="text-right pr-0">
                                    ...
                                    @if ($order->delivery_status == 'pending' && $order->payment_status == 'unpaid')
                                        <a  class="btn btn-soft-danger btn-icon btn-circle btn-sm hov-svg-white mt-2 mt-sm-0 " data-bs-toggle="modal" data-bs-target="#exampleModal" onclick="confirm_cancel_request('{{route('purchase_history.destroy', $order->id)}}')"   title="{{ translate('Cancel') }}">
            ...
            {{-- <div class="d-none col-md-6 mx-auto "  id="cancel_form">
                <form method="post" id="cancel_reason_form">
                    @csrf
                    <label for="">{{translate('Reason')}}</label>
                    <textarea class="form-control mb-3" rows="3" name="cancel_reason" placeholder="{{ translate('Reason') }}" required></textarea>
                    <button type="submit" class="btn btn-outline-danger">Xác nhân</button>
                </form>
            </div> --}}
...
@section('script')
    <script type="text/javascript">
        ...
        function confirm_cancel_request(url)
        {
            ...
            document.getElementById('cancel_reason_form').action = url;
        }
    </script>
@endsection
